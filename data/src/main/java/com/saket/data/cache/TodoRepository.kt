...
/**
 * The repository class is responsible to pass data between data and domain layers. It implements
 * ITodoRepository interface defined in the domain layer. It will use the todoTable to make the
 * actual requests. While the mapper will transform the data from data object to domain object.
 */
class TodoRepository constructor(private val todoTable: TodoTable) : ITodoRepository {
    override fun getTodoList(): Flow<List<Todo>> =
        todoTable.getAllTodos().map { list -> list.map { todoEntity -> todoEntity.todo } }
