...
class GetAllTodos constructor(
     ...
     */
    fun executeFlow(scope: CoroutineScope, block: (List<Todo>) -> Unit) {
        scope.launch {
            //todoRepository.getTodoList() is a flow returned from the db call.
            var job = todoRepository.getTodoList()
                .flowOn(executors)
                .catch { ex ->

                }
                .onCompletion {
                    println("$useCaseName has completed")
                }
                .collect {
                    block(it)
                }
